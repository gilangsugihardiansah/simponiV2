...
class SignatureWidget extends \yii\base\Widget
{
    ...
    public function init()
    {
        ...
        if ($this->clear == true) {
            $this->action_buttons .= '<button type="button" class="btn btn-info button clear" data-action="clear"><span class="glyphicon glyphicon-trash"></span></button>&nbsp;';
        }
        ...
        if ($this->undo == true) {
            $this->action_buttons .= '<button type="button" class="btn btn-info button" data-action="undo"><span class="glyphicon glyphicon-repeat"></span></button>';
        }
        if ($this->simpan == true) {
            $this->action_buttons .= '<button type="button" class="btn btn-info button" data-action="simpan" onclick="temporer(\'' . $this->url . '\')"><span class="glyphicon glyphicon-save"></span></button>';
        }
    ...
    public function run()
    {
        ...
        canvas{
         ...
         <div id="signature-pad" class="signature-pad">
            <div class="signature-pad--body" style="margin-bottom:10px;">
                <canvas></canvas>
            </div>
