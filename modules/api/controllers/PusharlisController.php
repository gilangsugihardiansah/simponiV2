...
use app\models\Spk;
use app\controllers\PusharlisLemburController;
...
class PusharlisController extends Controller
{
    ...
    public function actionSppd()
    {
        ...
        if($request->isPost):
            ...
            if( isset($bodys['nip']) AND isset($bodys['tgl_berangkat']) AND isset($bodys['tgl_kembali']) AND isset($bodys['kota_asal']) AND isset($bodys['kota_tujuan']) AND isset($bodys['instansi_tujuan']) AND isset($bodys['uraian_tugas']) AND isset($bodys['nama_atasan_pln']) AND isset($bodys['no_wa_atasan_pln']) AND isset($bodys['status_sppd']) AND isset($_FILES['evidence']['tmp_name']) ):
                $modelPeg = Pegawai::findOne($bodys['nip']);
                if(isset($modelPeg->NO_INDUK) AND $modelPeg->spkku->APP == "PUSHARLIS"):
                    ...
                    $model->STATUS_SPPD = $bodys['status_sppd'];
                    ...
                    $filename = 'files/pusharlis-sppd/'.$periode.'/'.str_replace('/','-',$model->ID).'.jpg';
                    ...
                    if($model->validate() && $model->save()):
                        ...
                        if (!file_exists('/var/www/html/sppdonln/web/files/pusharlis-sppd/'.$periode.'/')) :
                            mkdir('/var/www/html/sppdonln/web/files/pusharlis-sppd/'.$periode.'/', 0777, true);
                        endif;
        ...
        elseif($request->isGet):
            ...
            if(isset($params['nip']) AND isset($params['date'])):
                ...
                if(isset($modelPeg->NO_INDUK) AND $modelPeg->spkku->APP == "PUSHARLIS"):
                    $model = PusharlisSppd::find()->andWhere(['NO_INDUK'=>$params['nip']])->andWhere('CREATED_AT LIKE "'.$params['date'].'%"')->all();
    ...
    public function actionLembur()
    {
        ...
        if($request->isPost):
            ...
            if( isset($bodys['NO_INDUK']) AND isset($bodys['JAM_AWAL_LEMBUR']) AND isset($bodys['JAM_AKHIR_LEMBUR']) AND isset($bodys['JENIS_LEMBUR']) AND isset($bodys['PEKERJAAN_LEMBUR']) AND isset($bodys['ALAMAT']) AND isset($bodys['LATITUDE']) AND isset($bodys['LONGITUDE']) AND isset($bodys['URAIAN_TUGAS_LEMBUR']) AND isset($_FILES['EVIDENCE_LEMBUR']['tmp_name'])):
                ...
                if(isset($modelPeg->NO_INDUK)):
                    ...
                    $filename = 'files/pusharlis-lembur/'.$periode.'/'.str_replace('/','-',$model->ID).'-EL.jpg';
                    $model->EVIDENCE_LEMBUR = 'https://sppd.hapindo.co.id/'.$filename;

                    if($model->save()):

                        if (!file_exists('/var/www/html/sppdonln/web/files/pusharlis-lembur/'.$periode.'/')) :
                            mkdir('/var/www/html/sppdonln/web/files/pusharlis-lembur/'.$periode.'/', 0777, true);
                        endif;
        ...
        elseif($request->isGet):
            ...
            if(isset($params['NO_INDUK'])):
                ...
                if(isset($modelPeg->NO_INDUK) AND $modelPeg->spkku->APP == "PUSHARLIS"):
                    ...
                    $limit = !isset($params['LIMIT']) ? 15 : (intval($params['LIMIT']) < 0 ? 15 : $params['LIMIT']);
                    ...
                    if(isset($params['FIRST_DATE']) AND isset($params['LAST_DATE']) AND $params['FIRST_DATE'] != "" AND $params['LAST_DATE'] != "" ):
                        ...
                        $modelCount = PusharlisLembur::find()->andWhere(['NO_INDUK'=>$modelPeg->NO_INDUK])->andWhere(['>=','CREATED_AT',$fisrtDate])->andWhere(['<=','CREATED_AT',$lastDate])->count();
                        $model = PusharlisLembur::find()->andWhere(['NO_INDUK'=>$modelPeg->NO_INDUK])->andWhere(['>=','CREATED_AT',$fisrtDate])->andWhere(['<=','CREATED_AT',$lastDate])->limit($limit)->offset($offset)->orderBy(['CREATED_AT'=>SORT_DESC])->all();
                    else:
                        $modelCount = PusharlisLembur::find()->andWhere(['NO_INDUK'=>$modelPeg->NO_INDUK])->count();
                        $model = PusharlisLembur::find()->andWhere(['NO_INDUK'=>$modelPeg->NO_INDUK])->limit($limit)->offset($offset)->orderBy(['CREATED_AT'=>SORT_DESC])->all();
                    endif;
                    if(isset($model[0]) AND !empty($model)):
                        ...
                        foreach ($model as $key => $value):
                            ...
                            $res['data'][$key]['STATUS_PENGAJUAN'] = $value->status->VALUE;
                            ...
                            $res['data'][$key]['EVIDENCE_SPKL'] = PusharlisLemburController::actionGenerateSpklFromApi($value->ID);
            ...
            elseif(isset($params['ID'])):
                $res['value'] = 1;
                $res['message'] = 'Successfull';
                $res['data'] = PusharlisLembur::findOne($params['ID']);
                $res['data']['EVIDENCE_SPKL'] = PusharlisLemburController::actionGenerateSpklFromApi($params['ID']);
                return $res;
            else:
                Yii::$app->response->statusCode = 400;
                $res['value'] = 0;
    ...
    public function actionUpload()
    {
        ...
        if($request->isPost):
            ...
            if( isset($bodys['ID']) AND isset($_FILES['FILE']['tmp_name']) ):
                $model = PusharlisLembur::find()->andWhere(['ID'=>$bodys['ID']])->one();
                if(isset($model->ID) ):
                    $model->scenario = 'upload';
                    $periode = substr($model->CREATED_AT,0,7);
                    $filenameRealisasi = 'files/pusharlis-lembur/'.$periode.'/'.str_replace('/','-',$model->ID).'-ER.jpg';
